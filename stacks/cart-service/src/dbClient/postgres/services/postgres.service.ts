...
export class PostgresService implements IDbClientService {
  ...
  async query<T>(queryString: string, paramsValues?: unknown[]): Promise<IDbQueryResult<T>> {
    await this.connect();
  ...
  private async executeSqlStatement<T>(sql: string, paramsValues?: unknown[]): Promise<IDbQueryResult<T>> {
